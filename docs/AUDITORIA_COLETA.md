...
| TransfereGov — Transferências Especiais (Emenda Pix) | ⭐ **MIGRADO em 06/09/2026.** Listagem na API oficial `api-publica.transferegov.gestao.gov.br/especiais` (CNPJ → `id_beneficiario` → planos: 2 requisições por município, sem 403); pagamentos seguem na API interna da SPA, que é a única com CPF do ordenador e histórico de eventos da OP (`transferegov_te.py`) | 1/2 | **Feito.** Os ids são idênticos nas duas APIs em toda a cadeia (plano/empenho/DH/OP-OB, conferido campo a campo), então a PK não duplica e a fase de pagamentos não mudou. `situacao_trabalho` é traduzida de rótulo para o enum que o RM lê por substring (`_SITUACAO_PT_PARA_CODIGO`) — sem isso o plano sumiria do relatório em silêncio. | **Concluído** |
...
| Transparência MG (empenho/OB por credor) | HTTP stateful Joomla (`transparencia_mg.py`; **sem task e sem log** hoje) | 2 | **Parcial:** `pagamento{ano}.csv`/`pagamentorp{ano}.csv` SEGOV (semanal, chave SIAFI, 100 % de junção) — traz **valor empenhado/liquidado/pago** por NE (`valor_pago_financeiro`), mas **sem data/nº de OB/situação da OP**, que é o que o coletor grava. | Complementar (não substituir): carga do CSV para o grosso + Joomla sob demanda para OB específica. **Feito 15/09/2026 (Fase 1):** `ingestion/segov_pagamentos.py` → `segov_convenios_empenhos`, no cron do SIGCON após o backfill do CKAN; o RM imprime NEs, valor empenhado e "Empenhado: Sim" sempre a partir dele, e `Desembolsado`/`Pendente de desembolso` só quando o Joomla não respondeu. ⚠️ A junção 100 % acima é SEGOV×SEGOV; contra `convenios_estadual.nr_siafi` só a primeira carga mede (log "N de M casaram"). **Fase 2 feita no mesmo dia:** `ingestion/cge_despesa_ob.py` lê os dumps abertos da CGE (`despesa`: `ft_despesa_{ano}` + `dm_empenho_desp_{ano}`; `restos_pagar`) — a **mesma base** do Joomla (OB 1939 de Pequi idêntica, mesmos ids de empenho) — resolve as NEs da SEGOV por (nº, data) com **favorecido conferido pelo CNPJ** (`dm_favorecido`) e valor/soma de empenho como desempate — o nº de NE é sequencial por unidade executora, e sem o favorecido 22 NEs de terceiros casavam (achado da revisão adversarial) — e grava as OBs (nº · data · valor) em `transparencia_mg_empenhos` com o id do portal; RM e export passam a mostrar data/OB. **O que continua só no Joomla:** a situação bancária ("Acatada pelo banco") — a OB vai rotulada "OB emitida — confirmação bancária indisponível" e conta como desembolso por decisão do dono. | M |
