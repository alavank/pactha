...
/** ⚠️ `carregar` EXISTE PARA A TELA DE EMENDAS PARLAMENTARES (17/09/2026). Lá a
 *  proposta abre por `/emendas-parlamentares/emenda/parcerias/{id}`, que cobra a
 *  permissão de Emendas Federais — quem tem só aquela aba receberia 403 aqui.
 *  O payload é o mesmo (`dados` lá = a resposta daqui), então o modal é um só. */
export function DetalheProposta({ id, onFechar, carregar }: {
  id: number;
  onFechar: () => void;
  carregar?: (id: number) => Promise<DetalheResp>;
}) {
  ...
  useEffect(() => {
    let vivo = true;
    (carregar ? carregar(id) : api.get<DetalheResp>(`/parcerias/proposta/${id}`).then((r) => r.data))
      .then((d) => { if (vivo) setRes({ id, d, erro: null }); })
      .catch((e) => {
        if (vivo) setRes({ id, d: null, erro: e?.response?.data?.detail || "Não foi possível carregar." });
      });
    return () => { vivo = false; };
    // `carregar` fora das dependências de propósito: quem o passa costuma
    // escrevê-lo inline, e ele mudaria a cada render — o modal buscaria em laço.
