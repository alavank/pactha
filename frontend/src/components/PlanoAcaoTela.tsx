...
export default function TransfereGovPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <PlanoAcaoModal detalhe={detalhe} carregando={loadingDetalhe}
                      onFechar={() => setDetalhe(null)} />
    </div>
  );
}

/** O modal de UM plano de ação, fora da tela desde 17/09/2026.
 *
 *  ⭐ Separado para a tela de Emendas parlamentares abrir o MESMO modal com o
 *  MESMO payload (`/emendas-parlamentares/emenda/te/{id}` devolve em `dados`
 *  exatamente o que `/transferegov/plano-acao/{id}` devolve). Duas cópias do
 *  modal seriam duas versões do plano, e a primeira aba nova que entrasse numa
 *  delas faria as telas discordarem.
 *
 *  A aba e o pagamento aberto moram AQUI: são estado do modal, e quem o abre
 *  não precisa saber deles. */
export function PlanoAcaoModal({ detalhe, carregando: loadingDetalhe, onFechar }: {
  detalhe: DetalhePlano | null;
  carregando: boolean;
  onFechar: () => void;
}) {
  ...
  // Plano novo abre na primeira aba, como abria quando o estado era da tela.
  const idPlano = detalhe?.plano?.id_plano_acao ?? null;
  useEffect(() => { setTab("basicos"); setOpAberta(null); }, [idPlano]);
