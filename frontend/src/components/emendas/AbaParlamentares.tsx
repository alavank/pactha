...
interface ParlamentarItem {
  ...
  /** Partido, UF, cargo e foto (Câmara, Senado, ALMG). `null` = o nome não casou
   *  com ninguém, ou casou com duas pessoas — e a tela não afirma partido. */
  cadastro?: Cadastro | null;
}

export interface Cadastro {
  casa: string; id_externo: string; nome: string; partido: string | null;
  uf: string | null; cargo: string; foto_url: string | null;
}

/** Foto, partido-UF e cargo ao lado do nome. Sem cadastro, nada: um "sem
 *  partido" seria afirmar algo que a fonte não disse. */
export function SeloCadastro({ c }: { c?: Cadastro | null }) {
  if (!c) return null;
  return (
    <span className="inline-flex min-w-0 items-center gap-1.5">
      ...
      {(c.partido || c.uf) && (
        <span className="whitespace-nowrap rounded px-1 text-[10px] font-semibold"
              style={{ background: "var(--bi-line)", color: "var(--bi-muted)" }}>
          {[c.partido, c.uf].filter(Boolean).join("-")}
        </span>
      )}
      <span className="hidden whitespace-nowrap text-[10px] sm:inline"
...
function ParlamentaresInner() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      // ⭐ A ROTA DA TELA NOVA quando há município (17/09/2026): a MESMA soma de
      // `/parlamentares` (é a mesma função no servidor), com partido, cargo e
      // foto ao lado. Sem município (consolidado) segue a antiga, que aceita
      // o escopo inteiro.
      const r = await api.get<{ items: ParlamentarItem[]; contagem?: { parlamentar: number; outro: number } }>(
        municipioId ? "/emendas-parlamentares/parlamentares" : "/parlamentares", { params });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border-b pb-4" style={{ borderColor: "var(--bi-line)" }}>
        ...
        <h2 className="text-[15px] font-semibold">Parlamentares</h2>
