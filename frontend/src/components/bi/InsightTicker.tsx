...
export function InsightTicker({
  ...
  const texto = msgs[Math.min(i, msgs.length - 1)];

  return (
    <div
      className={cn("flex items-center gap-3 rounded-2xl px-3.5 py-2.5", className)}
    ...
    >
      ...
      <p
        key={`${aba}-${i}`}
        className={cn("bi-insight-enter min-w-0 flex-1 leading-snug", tv ? "text-[15px]" : "text-[13px]")}
