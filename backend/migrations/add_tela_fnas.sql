-- A tela «Assistência social — FNAS» chega a quem administra os usuários do
-- cliente (26/09/2026).
--
-- Tela nova não nasce concedida a ninguém (o defeito de 17/08: item filtrado por
-- `user_telas`, só o super-admin via). Mesmo desenho de `add_tela_pdde.sql`: a
...
SELECT user_id, 'fnas' FROM user_permissoes
WHERE permissao = 'usuarios.conceder'
  AND NOT EXISTS (SELECT 1 FROM user_telas WHERE tela = 'fnas')
ON CONFLICT DO NOTHING;

INSERT INTO user_permissoes (user_id, permissao)
SELECT user_id, 'fnas.ver' FROM user_permissoes
WHERE permissao = 'usuarios.conceder'
  AND EXISTS (SELECT 1 FROM permissoes_catalogo WHERE chave = 'fnas.ver')
  AND NOT EXISTS (SELECT 1 FROM user_permissoes WHERE permissao = 'fnas.ver')
ON CONFLICT DO NOTHING;
