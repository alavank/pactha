-- A tela «Defesa Civil e outros repasses (CGU)» chega a quem administra os
-- usuários do cliente (23/09/2026).
--
-- Tela nova não nasce concedida a ninguém (o defeito de 17/08: item filtrado por
-- `user_telas`, só o super-admin via). Mesmo desenho de `add_tela_tce_pr.sql`: a
...
SELECT user_id, 'cgu_convenios' FROM user_permissoes
WHERE permissao = 'usuarios.conceder'
  AND NOT EXISTS (SELECT 1 FROM user_telas WHERE tela = 'cgu_convenios')
ON CONFLICT DO NOTHING;

INSERT INTO user_permissoes (user_id, permissao)
SELECT user_id, 'cgu_convenios.ver' FROM user_permissoes
WHERE permissao = 'usuarios.conceder'
  AND EXISTS (SELECT 1 FROM permissoes_catalogo WHERE chave = 'cgu_convenios.ver')
  AND NOT EXISTS (SELECT 1 FROM user_permissoes WHERE permissao = 'cgu_convenios.ver')
ON CONFLICT DO NOTHING;
